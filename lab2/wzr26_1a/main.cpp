...
DWORD WINAPI ReceiveThreadFun(void *ptr)
{
	...
	while (1)
	{
		int frame_size = pmt_net->reciv((char*)&frame, &zmienna, sizeof(Frame));   // oczekiwanie na nadejœcie ramki 
...
void InteractionInitialisation()
{
	...
	uni_reciv = new unicast_net(1001);      // obiekt do odbioru ramek sieciowych
	uni_send = new unicast_net(1002);       // obiekt do wysy³ania ramek
...
void VirtualWorldCycle()
{
	...
	uni_send->send((char*)&frame, "192.168.1.17", sizeof(Frame));  // wys³anie komunikatu do pozosta³ych aplikacji
}
